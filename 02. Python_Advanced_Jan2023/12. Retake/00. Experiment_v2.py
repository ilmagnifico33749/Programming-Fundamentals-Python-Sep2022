...
while programmers_single_task_time_completion and number_of_tasks:
    current_programmer_time = programmers_single_task_time_completion.popleft()
    ...
    if current_calculation > ducks_max_value:
        programmers_single_task_time_completion.append(current_programmer_time)
        number_of_tasks.insert(0, (current_tasks - 2))
    else:
    ...
    ducks_rewarded_info = ""
    for duck, duck_info in ducks_dict.items():
        ducks_rewarded_info += f"{duck}: {duck_info['Given_Away']}\n"
    print(ducks_rewarded_info)
