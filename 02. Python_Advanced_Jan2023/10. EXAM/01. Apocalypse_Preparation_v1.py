...
def healing_items_creation(list_textiles, list_medicaments, my_dict):
    ...
    while len(textiles) > 0 and len(medicaments) > 0:
        ...
        for key in dict_healing_items.keys():
            del my_dict[key]["Value"]
            dict_healing_items[key] = dict_healing_items[key]['Number']
        sorted_dict = sorted(dict_healing_items.items(), key=lambda x: (-x[1], x[0]))
        for item in sorted_dict:
            healing_item = item[0]
            if dict_healing_items[healing_item] != 0:
                print(f"{healing_item} - {dict_healing_items[healing_item]}")
        if textiles:
