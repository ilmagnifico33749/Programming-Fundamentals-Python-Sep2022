...
def car_movement(command, car_current_position, matrix_size):
    ...
    if position_validity(car_new_position, matrix_size) is True:
        ...
        print(f"You can't go out of the race track!")
...
def race(race_track, car_current_position, current_km_passed, current_turn, race_finished):
    ...
    if race_finished is False and command != "End":
        ...
        print(f"Car Starting Position: {car_current_position}")
        print(f"Car new final position: {car_new_position}")
        print(f"Current KM Passed: {current_km_passed}")
        print(50 * "-")
        [print(" ".join(row)) for row in race_track]
        print(50 * "-")
        print(100 * "#")
        return race(race_track, car_new_position, current_km_passed, current_turn, race_finished)
...
turn = 0
race(matrix, [0, 0], km_passed, turn, race_finished=False)
